define(['canvas/util', 'canvas/control_point', 'canvas/end_point', 'canvas/path'],function(util, ControlPoint, EndPoint, Path){
    ...
    Pen.prototype.getCropImage = function(originCanvas, paths) {
        var newCanvas = document.createElement("canvas");
        newCanvas.height = paths[3].y - paths[0].y;
        newCanvas.width  = paths[1].x - paths[0].x;

        var origin_ctx = originCanvas.getContext("2d");
        var new_ctx = newCanvas.getContext("2d");

        new_ctx.putImageData(origin_ctx.getImageData(paths[0].x, paths[0].y, newCanvas.width, newCanvas.height), 0, 0);
    ...
    Pen.prototype.getMaskImage = function(originCanvas, img) {
        var newCanvas = document.createElement("canvas"),
            oriWidth = originCanvas.width,
            oriHeight = originCanvas.height;
        newCanvas.width = img.width;
        newCanvas.height = img.height;

        var origin_ctx = originCanvas.getContext('2d');
        var new_ctx = newCanvas.getContext('2d');
        var left = (oriWidth - img.width) / 2;
        var top = (oriHeight - img.height) / 2;
        new_ctx.putImageData(origin_ctx.getImageData((oriWidth - img.width) / 2, (oriHeight - img.height) / 2, img.width, img.height), 0, 0);
        return newCanvas;
    };

    Pen.prototype.drawColor = function(target, color) {
        var paths = this.paths, prev_ep, ep;
    ...
    Pen.prototype.deactive = function(callback) {
        var self = this;
    ...
    Pen.prototype.active = function(callback) {
        var self = this;
        listeners = {
